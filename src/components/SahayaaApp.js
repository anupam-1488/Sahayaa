...
import Donate from './Pages/Donate'; // New Donate Page
...
const SahayaaApp = () => {
  ...
  const handleSectionChange = (section) => {
    ...
    const routes = {
      'home': '/', 'about': '/team', 'members': '/members',
      'events': '/events', 'volunteers': '/volunteers',
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      const reverseRoutes = {
        '/': 'home', '/team': 'about', '/members': 'members',
        '/events': 'events', '/volunteers': 'volunteers',
        '/donate': 'donate', '/contact': 'contact'
      };
...
const checkAuthState = useCallback(async () => {
  try {
    // 1. Correct destructuring for Supabase v2
    ...
    setUser(null); // Fallback to guest mode on error
  }
}, []);
  ...
  const loadStats = async () => {
    const [e, v, o] = await Promise.all([
      db.getEventStats(), db.getVolunteerStats(), db.getOrganizationMemberStats()
    ]);
    setStats({
      ...DEFAULT_STATS,
 ...
 const loadDataForSection = useCallback(async (section) => {
  // Use a ref-based guard instead of state — avoids the re-render loop
  ...
  try {
    ...
    loadingRef.current.delete(section); // Allow future reloads
  }
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full min-h-[70vh]">
        {isCurrentSectionLoading ? (
          ...
          <div className="animate-in fade-in duration-500">
            ...
            {activeSection === 'about' && (
              <Team members={teamMembers} user={user} />
            )}
            
            {activeSection === 'members' && (
              <Members members={organizationMembers} user={user} />
            )}
...
{activeSection === 'events' && (
  <Events
    events={events}
    setEvents={setEvents}           // ← was missing
    user={user}
    ...
    onEventUpdate={async (id, updates) => {
      ...
      if (res.success) setEvents(prev => prev.map(e => e.id === id ? res.data[0] : e));
      return res;
    }}
...
const SectionLoader = ({ sectionName }) => (
  <div className="flex flex-col items-center justify-center py-32 text-center">
    ...
    <p className="text-gray-500 mt-2">Connecting you with Sahayaa Trust initiatives</p>
  </div>
);
...
const LoadingScreen = () => (
  <div className="min-h-screen bg-white flex items-center justify-center">
    <div className="text-center">
      ...
      <h2 className="text-3xl font-bold text-green-800 tracking-tight">SAHAYAA TRUST</h2>
